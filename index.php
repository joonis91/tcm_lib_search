...
try {
    ...
    error_log("index.php: 데이터베이스 연결 성공");
} catch (PDOException $e) {
    error_log("index.php: 데이터베이스 연결 실패: " . $e->getMessage());
...
error_log("index.php: 정규화된 검색어: '$search_query', 검색 단어: " . json_encode($search_terms));
...
try {
    if (!empty($search_terms)) {
        ...
        if ($selected_source) {
            ...
            error_log("index.php: 특정 출전 '$selected_source' 내 검색어 '$search_query'로 검색, 결과 개수: $total_items, 쿼리: $sql_count, 파라미터: " . json_encode($params));

            $stmt = $pdo->prepare($sql_select);
            $stmt->execute($params_select);
            $results = $stmt->fetchAll(PDO::FETCH_ASSOC);
            error_log("index.php: 특정 출전 검색 결과: " . json_encode($results));
        } else {
            // 전체 검색
            $sql_count = "
                SELECT COUNT(DISTINCT f.id)
            ...
            $stmt->execute($params);
            $total_items = $stmt->fetchColumn();
            error_log("index.php: 전체 검색, 검색어 '$search_query', 결과 개수: $total_items, 쿼리: $sql_count, 파라미터: " . json_encode($params));

            $stmt = $pdo->prepare($sql_select);
            $stmt->execute($params_select);
            $results = $stmt->fetchAll(PDO::FETCH_ASSOC);
            error_log("index.php: 전체 검색 결과: " . json_encode($results));
        }
        ...
        $total_items = $stmt->fetchColumn();
        error_log("index.php: 출전 '$selected_source'만 선택, 결과 개수: $total_items");
        ...
        $stmt->execute();
        $total_items = $stmt->fetchColumn();
        error_log("index.php: 기본 목록, 결과 개수: $total_items");
    ...
    error_log("index.php: 데이터 조회 실패: " . $e->getMessage() . ", 쿼리: " . ($stmt ? $stmt->queryString : 'N/A') . ", 검색어: '$search_query', 파라미터: " . json_encode($params ?? []));
...
<body>
                    ...
                    <div class="col-md-6 offset-md-3">
                        <form method="GET" action="index.php">
                            <div class="input-group">
                                <input type="text" class="form-control" name="search" placeholder="3단어 띄어쓰기 검색" value="<?php echo htmlspecialchars($search_query); ?>">
                ...
                <div id="results">
                    ...
                    <?php else: ?>
                        ...
                        <table class="table table-striped table-hover">
                            <thead>
                                <tr>
                                    <th scope="col">처방 ID</th>
                                    <th scope="col">약재</th>
                                    <th scope="col">출처</th>
                                    <th scope="col">원문 내용</th>
                                    <th scope="col">작업</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php foreach ($results as $row): ?>
                                    ...
                                    ?>
                                    <tr class="prescription" data-fangzi-id="<?php echo $fangzi_id; ?>">
                                        <td><?php echo $fangzi_id; ?></td>
                                        <td class="herbs">로딩 중...</td>
                                        <td class="source">로딩 중...</td>
                                        <td class="original-text">로딩 중...</td>
                                        <td>
                                            <a href="prescription.php?id=<?php echo $fangzi_id; ?>&source=fangzi" class="btn btn-primary btn-sm">자세히 보기</a>
                                            <a href="edit.php?id=<?php echo $fangzi_id; ?>" class="btn btn-warning btn-sm">수정</a>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
                        ...
                        ?>
                            <nav aria-label="Page navigation">
                                <ul class="pagination justify-content-center">
                                    <?php if ($page > 1): ?>
                                        <li class="page-item">
                                            <a class="page-link" href="index.php?<?php echo http_build_query(array_merge($_GET, ['page' => $page - 1])); ?>">이전</a>
                                        </li>
                                    <?php endif; ?>
                                    ...
                                    <?php if ($page < $total_pages): ?>
                                        <li class="page-item">
                                            <a class="page-link" href="index.php?<?php echo http_build_query(array_merge($_GET, ['page' => $page + 1])); ?>">다음</a>
                                        </li>
                                    <?php endif; ?>
                                </ul>
                            </nav>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (prescriptions.length === 0) {
            console.log('검색 결과가 없어 처방 데이터를 로드하지 않습니다.');
            return;
        }
        ...
        prescriptions.forEach(prescription => {
            ...
            if (!fangziId || fangziId === 'N/A') {
                console.warn(`유효하지 않은 fangziId: ${fangziId}`);
                return;
            }

            fetch(`fetch_snippet.php?fangzi_id=${fangziId}`)
                .then(response => {
                    console.log(`fetch_snippet.php 호출 응답 상태, fangzi_id: ${fangziId}, 상태: ${response.status}`);
                    if (!response.ok) throw new Error(`네트워크 응답이 실패했습니다: ${response.status}`);
                    return response.json();
                })
                .then(data => {
                    console.log(`fetch_snippet.php 데이터, fangzi_id: ${fangziId}`, data);
                    if (data.error) {
                        console.error(`fangzi_id ${fangziId}: ${data.error}`);
                        prescription.querySelector('.herbs').innerHTML = '<span class="text-danger">약재 로드 실패</span>';
                        prescription.querySelector('.source').innerHTML = '<span class="text-danger">출전 로드 실패</span>';
                        prescription.querySelector('.original-text').innerHTML = '<span class="text-danger">원문 로드 실패</span>';
                        return;
                    }
